...
SNRs = [0:5:25];
N_mc = 504;
Ks_estimate = zeros(length(SNRs),6);
...
%         Phase_relax_sort = Thitas_cal(3*k_initial,temp_index);
        ...
        AICs(1) = M*log(Var_s(1));
        BICs(1) = M*log(Var_s(1));
        for i = 2:k_initial+1
            BICs(i) = M*log(Var_s(i))+(5*(i-1))*log(M);
            AICs(i) = M*log(Var_s(i))+(10*(i-1));
        end
%         figure()
...
%         ylabel("Value")
    ...
    Ks_estimate(i_snr,1:3) = K_estimate;
    Ks_estimate(i_snr,4:6)= Probability/N_mc;
end
save('Ks_estimate_snr.mat','f','A','phase','SNRs','Ks_estimate')
